...
begin
  ...
  if v_kind = 'customer' then
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      into v_balances from public.customer_vaults
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      into v_balances from public.partner_accounts
     where partner_id = p_party_id
       and tenant_id = v_actor.tenant_id
       and active;
  else
    select coalesce(jsonb_agg(jsonb_build_object(
      'currency', balances.currency, 'amount', balances.amount
    ) order by balances.currency), '[]'::jsonb)
      into v_balances
      from (
        select cur_id currency, sum(amount) amount
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
   ...
   where tenant_id = v_actor.tenant_id
     and not deleted
     and ((cp_id = p_party_id) or (partner_id = p_party_id));
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
   ...
   where r.tenant_id = v_actor.tenant_id
     and (r.customer_id = p_party_id or r.partner_id = p_party_id);
  ...
  if v_kind in ('office', 'customer') then
    select coalesce(jsonb_agg(jsonb_build_object(
     ...
     where tenant_id = v_actor.tenant_id
       and (office_id = p_party_id or customer_id = p_party_id);
  end if;
...
begin
  ...
  from (
    select currency,
      coalesce((select sum(l.amount) from public.ledger l
        where l.tenant_id = v_actor.tenant_id and l.cur_id = c.currency), 0) physical,
      coalesce((select sum(a.amount) from public.account_ledger a
        where a.tenant_id = v_actor.tenant_id and a.cur_id = c.currency), 0) system,
      coalesce((select sum(v.available + v.reserved) from public.customer_vaults v
        where v.tenant_id = v_actor.tenant_id and v.currency = c.currency), 0) held,
      coalesce((select sum(d.outstanding_principal) from public.debts d
        where d.tenant_id = v_actor.tenant_id and d.currency = c.currency
          and d.status in ('open', 'partially_settled')), 0) debt
    from (
      select cur_id currency from public.ledger where tenant_id = v_actor.tenant_id
      union select cur_id from public.account_ledger where tenant_id = v_actor.tenant_id
      union select currency from public.customer_vaults where tenant_id = v_actor.tenant_id
      union select currency from public.debts
       where tenant_id = v_actor.tenant_id and status in ('open', 'partially_settled')
    ) c
  ) totals;
...
alter function public.sarraf_party_profile(text, text) owner to sarraf_definer;
alter function public.sarraf_cash_reconciliation() owner to sarraf_definer;
