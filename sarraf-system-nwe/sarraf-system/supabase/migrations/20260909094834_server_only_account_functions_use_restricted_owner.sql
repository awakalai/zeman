...
begin
  ...
  -- The server route created this Auth user immediately before this command and deletes it if
  -- the command fails. The database role deliberately has no access to the private auth schema.
...
alter function public.sarraf_manager_create_business_owner(text, text, text, text, uuid, text)
