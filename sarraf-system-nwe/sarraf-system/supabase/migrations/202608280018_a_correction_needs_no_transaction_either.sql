-- The last door in the room that was still locked.
--
-- 202608280015 let a receipt with no transaction be ACCEPTED. It did not let one be CORRECTED,
-- and the correction path has the same shape of test:
--
--   if v_currency is distinct from v_doc.expected_currency then
--     raise exception 'correction currency must match the assigned transaction';
--
-- A customer-seller's receipt has no assigned transaction, so expected_currency is null, so a
-- correction naming CNY is compared against nothing, is distinct from it, and is refused. Every
-- correction to every receipt that arrived on its own — which is the ordinary case, and the case
-- a receipt read by hand is always in — came back saying the currency did not match a
-- transaction that does not exist.
--
-- Nothing else changes: where a transaction IS assigned, the correction must still agree with it.
begin;
...
begin
  ...
  if p_action='correct' then
    ...
    -- Only where something expected a currency. A customer-seller's receipt arrives before any
    -- transaction exists, so expected_currency is null, and comparing a real currency against
    -- nothing refused every correction anybody tried to make to one of them.
