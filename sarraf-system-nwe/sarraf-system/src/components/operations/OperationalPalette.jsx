...
import { groupSearchResults, operationalSearch, safeCommand } from "../../services/operationalControl";
...
const GROUP_LABELS = {
  ku: { Command: "بەشەکان", customer: "کەس", partner: "هاوبەش", transaction: "مامەڵە", receipt: "فیش", intake: "بارکردن", batch: "کۆمەڵە", currency: "دراو" },
  en: { Command: "Navigation", customer: "People", partner: "Partners", transaction: "Transactions", receipt: "Receipts", intake: "Uploads", batch: "Batches", currency: "Currencies" },
  ar: { Command: "التنقل", customer: "الأشخاص", partner: "الشركاء", transaction: "المعاملات", receipt: "الإيصالات", intake: "المرفوعات", batch: "الدفعات", currency: "العملات" },
};
const groupLabel = (lang, type) => GROUP_LABELS[localeKey(lang)]?.[type] || type;
...
export function OperationalPalette({ client, lang = "ku", onNavigate = (path) => { window.location.hash = path.slice(1); } }) {
  ...
  // string through it.
  const choose = (item) => { if (!item || !safeCommand(item)) return; onNavigate(item.path, item.focus || null); setOpen(false); };
  const renderResults = grouped.flatMap((group) => [
    <li key={`group-${group.type}`} className="operation-search-group" role="presentation"><strong>{groupLabel(lang, group.type)}</strong><span>{group.results.length}</span></li>,
    ...group.results.map((item) => {
      const index = results.indexOf(item);
      return <li key={`${item.type}-${item.path}-${index}`} id={`operation-result-${index}`} role="option" aria-selected={active === index}><button type="button" onMouseEnter={() => setActive(index)} onClick={() => choose(item)}><strong>{item.label}</strong><span>{kindLabel(lang, item.type)}{item.context ? ` · ${item.context}` : ""}</span></button></li>;
    }),
  ]);
